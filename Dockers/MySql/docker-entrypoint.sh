...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			mysql_note "Creating Northwind database"
			mysql -h localhost -pmysql1337 -u root -r < /usr/share/northwind-default-current-timestamp.sql
			mysql -h localhost -pmysql1337 -u root -r northwind < /usr/share/northwind-data.sql 
			mysql_note "Northwind created succesfully"
